...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Extract Contact Information Bot - AI Writing Assistant for Bloggers</title>
        <!-- Fonts -->
    ...
    <body class="antialiased">
        ...
        <div class="relative flex items-top justify-center min-h-screen bg-gray-100 dark:bg-gray-900 sm:items-center py-4 sm:pt-0">
            <div class="max-w-6xl w-full mx-auto sm:px-6 lg:px-8 space-y-4 py-4">
                <div class="text-center text-gray-800 dark:text-gray-300 py-4">
                    <h1 class="text-7xl font-bold">Extract Contact Information Bot</h1>
                </div>
                <div class="w-full rounded-md bg-white border-2 border-gray-600 p-4 min-h-[60px] h-full text-gray-600">
                    <form id="generateText" class="inline-flex gap-2 w-full">
                        @csrf
                        <input required id="title" name="title" class="w-full outline-none text-2xl font-bold" placeholder="Type your sentence..." value="{{$title}}" />
                        <div>
        ...
        <script>
            ...
            $('#saveBtn').click(function (e) { 
                ...
                $.ajax({
                    type: "POST",
                    url: "{{route('extract-contact-information')}}",
                    data: {title:data},
                    dataType: "json",
